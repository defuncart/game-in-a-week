...
namespace DeFuncArt.Utilities
{
	...
	{
		/// <summary>A class representing a SupportedLanguage object.</summary>
		[System.Serializable]
		public class SupportedLanguage
		{
			...
			public SupportedLanguage(SystemLanguage language, TextAsset jsonDatabase)
			{
				this.language = language; this.jsonDatabase = jsonDatabase;
			}
		}
		/// <summary>An array of supported languages.</summary>
		[Tooltip("An array of supported languages.")]
		[SerializeField] protected SupportedLanguage[] supportedLanguages;
		...
		/// <summary>The number of localized languages.</summary>
		public int NUMBER_LOCALIZED_LANGUAGES
		{
			get { return supportedLanguages.Length; }
		}
		...
		private void OnValidate()
		{
			...
			Assert.IsTrue(supportedLanguages.Length > 0, "Need to supply at least one supported language.");
		}

		/// <summary>EDITOR ONLY: The asset's path.</summary>
		private static string assetPath
		{
			get { return "Assets/Imported/DeFuncArt/Prefabs/Managers/"+typeof(LocalizationManager).Name+".prefab"; }
		}

		/// <summary>EDITOR ONLY: An instance of the asset.</summary>
		public static LocalizationManager EDITOR_instance
		{
			get { return AssetDatabase.LoadAssetAtPath<LocalizationManager>(assetPath); }
		}
		...
		/// <summary>EDITOR ONLY: Sets the Supported Languages.</summary>
		/// <param name="supportedLanguages">Ann array of supported languages.</param>
		public void EDITOR_SetSupportedLanguages(SupportedLanguage[] supportedLanguages)
		{
			Assert.IsNotNull(supportedLanguages);
            for(int i=0; i < supportedLanguages.Length; i++)
            {
                Assert.IsNotNull(supportedLanguages[i]);
            }
		...
		/// <summary>Restarts the manager.</summary>
		public void Restart()
		{
		...
		public void SetLanguage(SystemLanguage language)
		{
			...
			{
				Debug.LogWarningFormat("{0} is not a supported language!", language.ToString());
				language = (matchSystemLanguage && language != Application.systemLanguage ? Application.systemLanguage : defaultLanguage);
				Debug.LogWarningFormat("Using {0} as the localized langauge instead.", language.ToString());
			}

			//determine the localized language index
			SupportedLanguage l = supportedLanguages.Where(x => x.language == language).ToList()[0];
			SettingsManager.instance.localizedLanguage = supportedLanguages.IndexOf<SupportedLanguage>(l);
		...
		public string StringForKey(LocalizationManagerKeys key)
		{
			return StringForKey(key.ToString());
		}
